...
rem  Sonuc ve HATALAR "gunluk_calisma.txt" dosyasina yazilir.
rem ============================================================
...
if not defined PY (
  echo. >> gunluk_calisma.txt
  echo [%date% %time%] HATA: pandas kurulu bir Python bulunamadi. >> gunluk_calisma.txt
  echo   Cozum: BASLAT.bat ile ana uygulamayi bir kez calistirin. >> gunluk_calisma.txt
  exit /b 1
)

echo. >> gunluk_calisma.txt
echo [%date% %time%] BASLIYOR - Python: !PY! >> gunluk_calisma.txt
!PY! gunluk_tarama.py >> gunluk_calisma.txt 2>&1
echo [%date% %time%] BITTI - cikis kodu: %errorlevel% >> gunluk_calisma.txt
exit /b %errorlevel%
